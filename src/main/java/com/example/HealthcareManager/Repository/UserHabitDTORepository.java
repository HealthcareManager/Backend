...
public interface UserHabitDTORepository extends JpaRepository<Habit, Integer> {
    ...
    @Query("SELECT new com.example.HealthcareManager.DTO.HabitDTO(u.id, h.alcohol, h.cigarette,  h.areca)"+
        "FROM User u JOIN Habit h ON u.id = h.user.id WHERE u.id = :userId")
    HabitDTO findHabitDTOByUserId(@Param("userId") String userId);
